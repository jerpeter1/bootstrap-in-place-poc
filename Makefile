...
MACHINE_NETWORK_ADDRESS := $(shell echo $(MACHINE_NETWORK) | cut -d'/' -f1)
MACHINE_NETWORK_PREFIX := $(shell echo $(MACHINE_NETWORK) | cut -d'/' -f2)
MACHINE_NETWORK_BASE_ADDRESS := $(shell echo $(MACHINE_NETWORK_ADDRESS) | cut -d'.' -f1-3)
...
$(NET_CONFIG): $(NET_CONFIG_TEMPLATE)
	sed -e 's/REPLACE_NET_NAME/$(NET_NAME)/' \
	    -e 's|CLUSTER_NAME|$(CLUSTER_NAME)|' \
	    -e 's|BASE_DOMAIN|$(BASE_DOMAIN)|' \
		-e 's/NET_UUID/$(NET_UUID)/g' \
		-e 's/BRIDGE_NAME/$(BRIDGE_NAME)/g' \
		-e 's/MACHINE_NETWORK_ADDRESS/$(MACHINE_NETWORK_ADDRESS)/g' \
		-e 's/MACHINE_NETWORK_BASE_ADDRESS/$(MACHINE_NETWORK_BASE_ADDRESS)/g' \
		-e 's/MACHINE_NETWORK_PREFIX/$(MACHINE_NETWORK_PREFIX)/g' \
